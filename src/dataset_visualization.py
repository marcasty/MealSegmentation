...
def add_masks_and_labels(img_dir, mask_dir, data_file):
    ...
    for id, annot in metadata.coco["images"].items():
        ...
        plot_boxes(
            image_rgb, boxes, box_confidence, classes, class_ids, save_filename_box
        )
        multiplot_boxes(
            image_rgb, boxes, box_confidence, classes, class_ids, save_filename_multibox
        )
        plot_masks(
            image_rgb, mask, scores, classes, class_ids, mask_dir, save_filename_mask
        )
        multiplot_masks(
            image_rgb,
            mask,
...
def plot_boxes(
    img, bboxes, box_confidence, classes, class_ids, filename, ax=None, figsize=(7, 11)
):
...
def plot_masks(
    ...
    for i in range(len(masks_list)):
        food_mask = (
            torch.load(os.path.join(load_path, masks_list[i])).int().detach().numpy()
        )
...
def multiplot_masks(
    ...
    if num_objects > 1:
        ...
        for i, axs in enumerate(ax):
            food_mask = (
                torch.load(os.path.join(load_path, masks_list[i]))
                .int()
                .detach()
                .numpy()
            )
...
def multiplot_boxes(
    img, boxes, box_confidence, classes, class_ids, filename, figsize=(20, 5)
):
