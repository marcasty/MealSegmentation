    ...
    for ann_id, ann in coco.anns.items():
        if "bbox" not in ann:
            if ann_id in coco.dataset["info"]["detection_issues"]["failures"]:
                ...
                print(f"Annotation {ann_id} is missing a bbox, but it is *unaccounted* for")
                fail = 1
                count += 1
            
    if fail == 1: 
        raise AssertionError(f"Your pipeline didn't box {count} images without telling you")
    else:
        print(f"Test Passed! You either boxed all images or know of the ones without boxes")
