...
llava_tokenizer_, llava_model_, llava_image_processor_, _ = load_pretrained_model(LLAVA_MODEL, None, model_name, load_8bit=False, load_4bit=False, device_map="auto", device=DEVICE)
for i in range(20):
    start = time.time()
    generated_text = run_llava15(input_tensor, llava_tokenizer=llava_tokenizer_, llava_model=llava_model_, llava_image_processor=llava_image_processor_, temperature=0.5)
