...
def check_missing_masks(coco: FoodMetadata) -> None:
    ...
    for ann_id, ann in coco.anns.items():
        if "mask" not in ann and ann_id not in coco.dataset["info"]["detection_issues"]["failures"]:
            print(f"Annotation {ann_id} is missing a mask!")
            fail = 1
            count += 1
    if fail == 1: 
        raise AssertionError(f"Your pipeline didn't produce masks for {count} boxed images")
    else:
